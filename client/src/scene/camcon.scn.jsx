...
export const CamCon = ({ core }) => {
    ...
    useEffect(() => {
        if (SSUI.history.index === 0) {
            gsap.to(camera.current.position, { duration: 2.5, delay: 0.5, x: 0, y: 1, z: 12 })
        } else if (SSUI.value.name === 'Game') {
            if (SSGame.ui === 'Winner') {
                controls.current.autoRotate = false
                gsap.to(camera.current.position, { duration: 2.5, delay: 0.5, x: 0, y: 2, z: 14 })
            } else if (SSGame.ui === 'Board') {
                controls.current.autoRotate = true
                gsap.to(camera.current.position, { duration: 2, x: 0, y: 1, z: 10 + SSIndicator.players.all })
            } else {
                gsap.to(camera.current.position, { duration: 2, x: 0, y: 10 + SSIndicator.players.all * 2.5, z: 0 })
                gsap.to(controls.current.rotation, { duration: 2, x: 0, y: 0, z: 0 })
            }
        } else if (SSUI.value.name !== 'Game') {
            gsap.to(camera.current.position, { duration: 2, x: 0, y: 1, z: 10 + SSIndicator.players.all })
        } else {
            gsap.to(camera.current.position, { duration: 2, x: 0, y: 1, z: 12 })
        }
