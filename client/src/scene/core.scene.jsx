...
export const Scene = ({ core }) => {
    ...
    const CamCon = () => {
        ...
        return (
            <>
                <PerspectiveCamera ref={camera} position={[0, 1, core.isMobile ? 80 : appSnap.isInLobby ? 10 + appSnap.lobby.props.players.all : 12]} fov={30} near={0.01} far={1500} makeDefault />
    ...
    const Player = ({ player, position, rotation }) => {
        ...
        const lod = 32
        const isReady = player !== 'Waiting...'
        const color = genColor()


        const Material = () => {
            if (isReady) return <meshStandardMaterial color={color} metalness={0.2} roughness={0} />
    ...
    const Avatar = () => {
        ...
        return !appSnap.isInLobby && <Player player={appSnap.profile.username} position={[0, 0, 0]} />
    }
    ...
    const Players = () => {
        ...
        const players = appSnap.lobby.props.players
